...
Otherwise, the resulting form will evaluate direcly to the function."
  ([cont env name & bodies]
    (letfn [(transform-fn [name bodies]
              (let [return (gensym)
                    env (gensym "env_")
      ...
      (cond
        ;; if no name, use nil
        (not (or (symbol? name)
                 (nil? name))) `(cps-fn* ~cont ~env nil ~name ~@bodies)
        ;; normalize (fn* [.] ...) form to (fn* ([.] ...)) form
        (vector? (first bodies)) `(cps-fn* ~cont ~env ~name (~@bodies))
        ;; normal form => transform
        :else (transform-fn name bodies)))))
